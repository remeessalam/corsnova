...
const BlogItem = ({ item, passkey }) => {
  ...
  return (
    <div className="flex flex-col h-full border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden group hover:shadow-lg transition-all duration-300">
      ...
      <div className="flex-1 flex flex-col p-5 text-black">
        {/* Category and Date */}
        <div className="flex justify-between items-center mb-3">
          {item.categoryId && (
            <span className="text-xs font-medium bg-gray-500 text-primary py-1 px-3 rounded-full">
        ...
        <Link to={`/blog/${item.slug}`}>
          <h3 className="text-lg font-semibold text-black line-clamp-2 group-hover:text-primary duration-200 transition-all mb-2">
            {item.title}
          </h3>
        </Link>

        {/* Excerpt */}
        <p className="line-clamp-3 text-black mb-4 flex-grow">{item.excerpt}</p>
